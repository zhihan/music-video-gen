...
class ImagenClient:
    ...
    Uses the Vertex AI predict endpoint.
    """
    ...
    def __init__(
        ...
        self._model = (
            model
            or getattr(config, "imagen_model", None)
    ...
    def generate_image(
        ...
        try:
            ...
            logger.info(
                "Generating image with Imagen: %s...",
                prompt[:50],
            )
            ...
            debug_file = Path(
                f"imagen_response_{int(time.time())}.json"
            )
            with open(debug_file, "w") as f:
                json.dump(data, f, indent=2, default=str)
            logger.debug(
                "Saved response to %s", debug_file
            )
